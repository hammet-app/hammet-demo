...
export default function ClassStudentsWithArmPage() {
  const { accessToken, refreshToken } = useAuth();
  const router = useRouter();
  const params = useParams<{ level: string; arm: string }>();
  const level = decodeURIComponent(params.level);
  const arm = decodeURIComponent(params.arm);
  ...
  useEffect(() => {
    ...
    getClassStudents(level, arm, accessToken, refreshToken)
      .then((res) => {
  ...
  }, [accessToken, refreshToken, level, arm]);

  const title = `${level} ${arm}`;
  const subtitle =
    isLoading ? "Loading..." : term != null ? `Term ${term}` : undefined;
