...
export function SchoolCard({
  ...
  return (
    <div
      ...
      `}
      onClick={() =>
        router.push(`/hammet/schools/${school.id}`)
      }
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3">
            <div
              className={`flex h-10 w-10 items-center justify-center rounded-xl ${tier.iconBg}`}
            >
              <Building2
                size={20}
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-[var(--color-text-muted)]">
                Since{" "} {new Date(school.createdAt).getFullYear()} • Term {school.term}
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div
            key={label}
            className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-page)] px-4 py-3 text-center"
          >
